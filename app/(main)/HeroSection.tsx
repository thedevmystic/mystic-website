...
export default function HeroSection() {
      ...
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          setScrollCueState((prevState) => ({
            initial: prevState.initial ? !isScrolled : false,
            scrolled: isScrolled,
          }));
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden text-center py-6 z-10">
      <HomepageAnimation />
      ...
      <div
        className={`absolute inset-x-0 bottom-[34px] z-10 transition-opacity duration-300 ease-out ${
          scrollCueState.scrolled ? 'opacity-0 pointer-events-none' : 'opacity-100'
        }`}
      >
        <div
          className={`w-max mx-auto ${
            scrollCueState.initial
              ? 'opacity-0 animate-[riseIn_0.8s_ease_forwards] [animation-delay:1.6s]'
              : ''
          }`}
        >
          <div className="flex flex-col items-center justify-center gap-y-2 text-xs text-on-surface-variant font-mono tracking-wide uppercase animate-[bob_2.4s_ease-in-out_infinite]">
            Scroll
